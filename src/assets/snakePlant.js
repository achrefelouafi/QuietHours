...
(QH => {
  ...
  QH.assets.snakePlant = (x, y, z, o = {}) => {
    ...
    for (let i = 0; i < n; i++) leaves.push({ a: R() * TAU, l: size * (0.55 + R() * 0.5), o: 0.05 + R() * r * 0.5, w: 0.09 + R() * 0.06, lt: R() > 0.55, bend: 0.25 + R() * 0.3 });
    ...
    for (const L of leaves) {
      ...
      const m = [x + ux * (L.o + L.l * L.bend * 0.5), y + uy * (L.o + L.l * L.bend * 0.5), zb + L.l * 0.62];
      const t = [x + ux * (L.o + L.l * L.bend), y + uy * (L.o + L.l * L.bend), zb + L.l];
      beam(b, m, L.w, c, 1);
