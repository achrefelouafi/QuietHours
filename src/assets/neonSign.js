...
(QH => {
  ...
  const neonSign = (wall, u, z, w, o = {}) => {
    const Wl = W[wall], h = o.h || 0.9, off = 0.14;
    ...
    for (let i = 1; i < pts.length; i++) beam(pts[i - 1], pts[i], 0.42, M.rustDk, 1);
    for (let i = 1; i < pts.length; i++) beam(pts[i - 1], pts[i], 0.19, M.bright, 1);
    for (let i = 1; i < pts.length; i++) beam(pts[i - 1], pts[i], 0.08, M.glow, 1);
