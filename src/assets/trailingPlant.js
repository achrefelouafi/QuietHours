...
(QH => {
  ...
  QH.assets.trailingPlant = (x, y, z, o = {}) => {
    ...
    const zb = z + ph - 0.03, strands = [], S = sway.plant('trailingPlant', x, y, zb, drop, -0.4);   // its middle hangs below the pot
    for (let i = 0; i < n; i++) {
      const a = base + (i / (n - 1) - 0.5) * TAU * 0.62 + (R() - 0.5) * 0.3;
      const off = Math.cos(a - base);                                  // 1 straight out, 0 sideways
      strands.push({ i, a, out: 0.35 + R() * 0.3 + off * 0.2, fall: off > 0.1 ? drop * (0.45 + R() * 0.55) : 0.15 + R() * 0.15, lt: R() > 0.5 });
    }
    ...
    for (const s of strands) {
      ...
      const at = t => S.at(s.i, [(1 - t) * (1 - t) * p0[0] + 2 * (1 - t) * t * p1[0] + t * t * p2[0],
                                 (1 - t) * (1 - t) * p0[1] + 2 * (1 - t) * t * p1[1] + t * t * p2[1],
                                 (1 - t) * (1 - t) * p0[2] + 2 * (1 - t) * t * p1[2] + t * t * p2[2]], t * Math.min(1, s.fall / drop + 0.3));
    ...
    for (let i = 0; i < 5; i++) {
      ...
      beam([x, y, zb], S.at(n + i, [x + Math.cos(a) * l, y + Math.sin(a) * l, zb + l * 1.1], 0.3), 0.07, i % 2 ? M.leaf : M.leafLt, 1);
    }
  };
})(QH);
