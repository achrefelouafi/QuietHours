...
(QH => {
  ...
  QH.assets.monstera = (x, y, z, o = {}) => {
    ...
    const zb = z + ph - 0.05, leaves = [];
    for (let i = 0; i < n; i++) {
      const a = (i + R() * 0.6) / n * TAU;
      leaves.push({ a, h: size * (0.5 + R() * 0.5), out: 0.45 + R() * 0.55, s: 0.7 + R() * 0.4, lt: R() > 0.5, tilt: 0.15 + R() * 0.3 });
    }
    ...
    for (const L of leaves) {
      ...
      const c = [x + ux * L.out, y + uy * L.out, zb + L.h];                          // where the leaf starts
      ...
      for (const k of [-2, 2, -1, 1, 0]) {
        ...
        const tip = [c[0] + Math.cos(a) * len, c[1] + Math.sin(a) * len, c[2] - len * L.tilt - Math.abs(k) * 0.05];
        beam(c, tip, 0.2, col, 1);
        beam([c[0] + Math.cos(a) * len * 0.35, c[1] + Math.sin(a) * len * 0.35, c[2] - len * L.tilt * 0.35], tip, 0.13, col, 1.06);
      }
      beam(c, [c[0] + ux * L.s * 0.85, c[1] + uy * L.s * 0.85, c[2] - L.s * L.tilt * 0.85], 0.03, M.leafDk, 1);   // midrib
    }
  };
})(QH);
