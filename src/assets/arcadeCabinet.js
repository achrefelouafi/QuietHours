/* ═══════════════════════════════════════════════════════════════
   ...
   arcadeCabinet(x, y, t, o)
     ...
     top of the marquee. o.on (1) is how lit the screen and the
     marquee are, 0..1: at 0 the screen is dark glass with nothing
     scrolling, the marquee is dark and only its lettering reads.
     Returns { quad, mask }: the screen rectangle on screen, for the
     click, and the cabinet's outline for draw.mask in the lighting
     pass so the neon's halo stays off it — a glow over the blue
     dithers into speckle.
   ═══════════════════════════════════════════════════════════════ */
(QH => {
  const M = QH.M;
  ...
  QH.assets.arcadeCabinet = (x, y, t = 0, o = {}) => {
    const face = o.face || '+x', wd = o.w || 1.2, dp = o.d || 1.35, h = o.h || 3.3, on = o.on ?? 1;
    const X = face === '+x';
    ...
    const glass = mix(M.ink, M.blueLt, on);                                   // dark glass off, cyan-blue lit
    const gb = fb + 0.01, ga0 = wd * 0.14, ga1 = wd * 0.86, gz0 = 2.12, gz1 = 2.67;   // the glass: its face, across and up
    front(gb, ga0, gz0, ga1, gz1, rgb(glass));
    const quad = [fp(gb, ga0, gz0), fp(gb, ga1, gz0), fp(gb, ga1, gz1), fp(gb, ga0, gz1)].map(p => P(...p));   // the glass on screen, for the click
    if (on > 0) {
      ...
      for (let i = 0; i < rows; i++) {
        ...
        front(fb + 0.02, wd * 0.2 + ind, zz, wd * 0.2 + ind + len, zz + 0.05, rgb(mix(glass, R() > 0.75 ? M.cyan : M.blue, on)));
      }
      if (Math.floor(t * 2) % 2 === 0) front(fb + 0.02, wd * 0.66, 2.21, wd * 0.74, 2.28, rgb(mix(glass, M.ice, on)));
    }
    ...
    front(mb, wd * 0.1, 3.0, wd * 0.9, h - 0.14, rgb(mix(M.ink, M.cyan, on)));                       // the lit face fades to dark glass off
    ...
    const fd = dp + jut;
    return { quad, mask: [fp(0, 0, h), fp(fd, 0, h), fp(fd, 0, 0), fp(fd, wd, 0), fp(0, wd, 0), fp(0, wd, h)] };
  };
})(QH);
