/* ═══════════════════════════════════════════════════════════════
   ...
   seat cushion and one tall green back cushion the full width of the
   frame, which the arm rails run into and the rear posts hide inside.
...
(QH => {
  ...
  const RAIL = 1.72, STRETCH = 0.6, SEAT = 0.85, CUSH = 0.48, BT = 0.55, BACK_H = 2.7;   // BT: the back cushion's thickness
  ...
  QH.assets.frameChair = (x, y, o = {}) => {
    ...
    const side = a => {                                                  // one side of the frame: the stretcher, the front post, then the arm rail on top of it
      bx(a, P, STRETCH - 0.14, P, D - 2 * P, 0.14, timber, wood);
      post(a, D - P, RAIL);                                              // before the rail: the rail sits on it, so its top must not show through
      bx(a, P, RAIL, P, D - P, 0.18, timber, wood);                       // from the rear post's face to the front edge
    };

    post(0, 0, RAIL); post(W - P, 0, RAIL);                              // the rear posts, inside the back cushion above the seat
    side(0);                                                             // the far side, behind the cushions
    bx(0, 0, SEAT + 0.05, W, BT, BACK_H - SEAT - 0.05, M.sage, green);   // the back cushion: one box the full width, over the far arm rail
    ...
    bx(P, BT, SEAT, W - P, D - BT - P + 0.06, CUSH, M.sage, green);                          // the seat cushion, out to the near edge under the arm rail and over the front rail a little
