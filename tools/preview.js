 ...
 *
 *   node tools/preview.js [outfile] [seconds] [width] [height] [lamp] [zoom] [panX] [panY] [room] [neon] [strip] [flash] [bath] [show] [duvet] [chair] [pc] [mixer] [arcade] [record]
 *
 ...
 * booth at rest). `record` is how many seconds ago the record downstairs
 * was put on (its ripples and the speakers' bounce come up over six
 * tenths; leave it out for the turntable at rest).
 *
...
const MIXER = parseFloat(process.argv[19] || '1');
const ARCADE = parseFloat(process.argv[20] || '1');
const RECORD = process.argv[21];
...
const stubEl = { textContent: '', style: {}, hidden: true, addEventListener: () => {}, querySelectorAll: () => [], querySelector: () => null, classList: { toggle() {}, remove() {}, add() {} }, setAttribute: () => {}, focus: () => {} };   // the slate, the HUD, the record popup
...
if (MIXER < 1) app.setMixer(false);
if (ARCADE < 1) app.setArcade(false);
...
if (RECORD !== undefined) app.playRecord('', parseFloat(RECORD));
